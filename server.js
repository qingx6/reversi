...
function adjacent_support(who, dr, dc, r, c, board) {
    ...
    if (who === 'b'){
        ...
        log("While calculating valid moves, we received " + who + " for 'who', which is not 'w' or 'b'");
        return false;
    }
    ...
    /* Check that there is space for your token to be placed */
...
function calculate_legal_moves(who, board){
    ...
    for(let row = 0; row < 8; row++){
        for(let column = 0; column < 8; column++) {
            if (board[row][column] === ' ' ){
                /* Go through each of the cardinal directions (north = nn, southeast = se, etc.) and check if we can play in that direction */
                nw = adjacent_support(who, -1, -1, row, column, board); /* Can we play in the spot directly above us, i.e. spot (row-1, column)? */
